...
**v3.0.0** | [更新日志](CHANGELOG.md) | [架构文档](ARCHITECTURE.md) | [English](README.md)
...
| **setup** | `/ham-autocode:setup` | 自动检测并安装缺失的依赖 skill pack |

---

## Core Engine CLI

Core Engine 是 TypeScript CLI（零运行时依赖），Skill 通过它管理状态、调度任务和路由决策。
...
```bash
node dist/index.js learn analyze              # 分析 trace + 任务历史 → 生成洞察
node dist/index.js learn suggest              # 预览阈值自适应建议
node dist/index.js learn apply                # 将建议写入 harness.json
node dist/index.js learn patterns             # 扫描项目结构 + 任务类型模式
node dist/index.js learn hints <task-name>    # 获取基于历史模式的任务提示
node dist/index.js learn history              # 查看学习趋势
node dist/index.js learn reset                # 清除所有学习数据
```
...
ham-autocode 编排以下框架 Skill Pack。使用 `/ham-autocode:setup` 自动检测并安装缺失的依赖。

| 依赖 | Stars | 角色 | 用于 | 安装命令 |
|------|-------|------|------|---------|
| **[GSD](https://github.com/gsd-build/get-shit-done)** | 39K+ | 稳定层 | Phase 2-4：项目初始化、里程碑、自主执行 | `git clone --depth 1 https://github.com/gsd-build/get-shit-done.git ~/.claude/plugins/gsd` |
| **[gstack](https://github.com/garrytan/gstack)** | 66K+ | 决策层 | Phase 1/5/6：立项审查、QA 测试、发布 | `git clone --depth 1 https://github.com/garrytan/gstack.git ~/.claude/skills/gstack` |
| **[Superpowers](https://github.com/obra/superpowers)** | 120K+ | 执行层 | Phase 4：TDD 执行方法论 | `git clone --depth 1 https://github.com/obra/superpowers.git ~/.claude/plugins/superpowers` |
| **[OpenSpec](https://github.com/Fission-AI/OpenSpec)** | 39K+ | 规范层 | 规范驱动路由（可选，提升任务评分） | `git clone --depth 1 https://github.com/Fission-AI/OpenSpec.git ~/.claude/plugins/openspec` |

安装后在 Claude Code 中运行 `/reload-plugins`。如果某个依赖未安装，流水线会优雅地报告错误并继续。
