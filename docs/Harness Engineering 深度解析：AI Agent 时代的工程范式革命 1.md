...
author:
  - "[[Meta​​公众号：Meta的LLM笔记。关注Agent/LLM4Code/LLM infra]]"
published:
created: 2026-04-15
...
tags:
  - "clippings"
---
2026 年 2 月，"Harness Engineering"这个词突然在 AI 工程圈子里火了起来。Mitchell Hashimoto 在博客里提了这个说法，OpenAI 紧接着发了百万行代码的 [实验报告](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E5%AE%9E%E9%AA%8C%E6%8A%A5%E5%91%8A&zhida_source=entity) ，Martin Fowler 也跟进写了深度分析——几周之内，这个术语就成了讨论 AI Agent 开发绕不开的话题。本文把 Harness Engineering 的来龙去脉、核心方法和实战经验做了一次系统整理，重点拆解 OpenAI、Anthropic、Stripe 等团队踩过的坑和沉淀下来的做法。最后，我们对 8 个独立信息源进行交叉对比，梳理出业界已形成共识的六大方面、仍存分歧的四个议题，以及三个最值得探索的空白区。
...
Phil Schmid 打了个比方： **模型是 CPU，Harness 是 [操作系统](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F&zhida_source=entity)** ——CPU 再强，OS 拉胯也白搭。mtrajan 的区分更直接：Context Engineering 管的是"给 Agent 看什么"，Harness Engineering 管的是"系统怎么防崩、怎么量化、怎么修".
...
综合 OpenAI、Anthropic、Carlini（C [编译器](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E7%BC%96%E8%AF%91%E5%99%A8&zhida_source=entity) 项目）、Huntley、Horthy 等五个独立团队的实践，四种模式反复出现并形成收敛。这就是 Harness Engineering 的四大支柱。
...
**原则 3：将代码仓库作为唯一事实源。** 写在 Slack 讨论或 Google Docs 中的知识对 Agent 来说等于不存在。所有团队知识都作为 [版本控制](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E7%89%88%E6%9C%AC%E6%8E%A7%E5%88%B6&zhida_source=entity) 的制品放置在仓库中。

**原则 4：将 [可观测性](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E5%8F%AF%E8%A7%82%E6%B5%8B%E6%80%A7&zhida_source=entity) 连接到 Agent。** 他们将 Chrome DevTools 连接到运行时，使 Agent 能够捕获 DOM 快照和截图。通过赋予查询日志和指标的能力，"将启动时间降至 800ms 以下"变成了可度量的目标。

**原则 5：对抗熵。** 最初团队每周五花 20% 的时间手动清理"AI Slop"（低质量生成物）。这后来被自动化为 Codex 运行的后台任务——清理 [吞吐量](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E5%90%9E%E5%90%90%E9%87%8F&zhida_source=entity) 与代码生成吞吐量成正比扩展。
...
**浏览器 [自动化测试](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E8%87%AA%E5%8A%A8%E5%8C%96%E6%B5%8B%E8%AF%95&zhida_source=entity)** ：为 Claude 提供 Puppeteer MCP 等测试工具后，Agent 能够识别和修复仅从代码层面无法看到的 bug，显著提升了端到端验证的效果。
...
**隔离的预热 Devbox** ：与人类工程师使用相同的 [开发环境](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E5%BC%80%E5%8F%91%E7%8E%AF%E5%A2%83&zhida_source=entity) ，但与生产和互联网隔离。
...
OpenAI 团队讨论了架构刚性和执行规则。关键聚焦领域： **保持 [数据结构](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84&zhida_source=entity) 稳定** 和 **定义并执行模块边界** 。
...
Harness Engineering 标志着 AI 辅助开发从"让模型写代码"到"设计让模型可靠工作的系统"的范式转变。这不是等更强模型出来就能解决的事——模型越强，Harness 反而越重要。六大共识已经清晰（基础设施是瓶颈、文档要活、思考与执行分离、上下文不是越多越好、约束必须自动化、工程师角色在转变），但棕地项目改造、功能验证体系、AI 代码的长期可维护性三大空白区仍待填补。如果只记一句话： **瓶颈不在智能，而在基础设施。** 正如 Addy Osmani 所言："AI 编码的兴起并没有取代 [软件工程](https://zhida.zhihu.com/search?content_id=271147250&content_type=Article&match_order=1&q=%E8%BD%AF%E4%BB%B6%E5%B7%A5%E7%A8%8B&zhida_source=entity) 的工艺——它抬高了工艺的门槛。"
...
发布于 2026-03-08 16:30・河北・包含 AI 辅助创作 作者对内容负责